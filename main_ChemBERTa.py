...
def evaluate(model, valid_iter, vocabulary, tokenizer, device, epoch,EMB_SIZE, loss_fn=None):
    model.eval()
    total_loss = 0
    n_batches = 0
    with torch.no_grad():
        for batch in valid_iter:
            ...
            if _target is None:
                ...
                target_stu = torch.FloatTensor(np.stack(targetemb_stu)).to(device) ##sed##
            ...
            raw_affinity_sum += current_raw_aff
            n_batches += 1
...
def train_epoch(model, train_iter, optimizer, vocabulary, tokenizer, device, epoch,EMB_SIZE, loss_fn=None):
    model.train()
    total_loss = 0
    ...
    avg_raw_aff = raw_affinity_sum / len(train_iter)
    wandb.log({
...
def greedy_decode(model, max_len, start_symbol, EOS_IDX, target, device, EMB_SIZE):
    ...
    for i in range(max_len-1):
        ...
        if target == "0":
            ...
            _target = torch.FloatTensor(seqid_to_emb[t]).unsqueeze(0).to(device)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--d_model', default=768, type=int)
    ...
    wandb.init(
    project="drug-generation_2",  
    name="exp2_Chemberta_base",                        
    config={
        "learning_rate": lr,
    ...
    if args.mode == 'train':
        ...
        for epoch in range(start_epoch, NUM_EPOCHS+1):
            ...
            train_loss = train_epoch(transformer, train_iter, optimizer, vocabulary, mv.SMILESTokenizer(), DEVICE, epoch,EMB_SIZE, loss_fn)
            scheduler.step()
            end_time = time.time()
            
            val_loss = evaluate(transformer, valid_iter, vocabulary, mv.SMILESTokenizer(), DEVICE,epoch,EMB_SIZE, loss_fn)
        ...
        state_dict = torch.load("model_base_1024.h5")
        ...
        nn.init.xavier_uniform_(transformer.proj_protein.weight)
        nn.init.zeros_(transformer.proj_protein.bias)
        ...
        for epoch in range(start_epoch, NUM_EPOCHS+1):
            start_time = time.time()
            train_loss = train_epoch(transformer, train_iter, optimizer, vocabulary, mv.SMILESTokenizer(), DEVICE, epoch,EMB_SIZE, loss_fn=loss_fn)

            scheduler.step()
            end_time = time.time()

            val_loss = evaluate(transformer, val_iter, vocabulary, mv.SMILESTokenizer(), DEVICE,epoch, EMB_SIZE, loss_fn=loss_fn)
            if val_loss < min_loss:
                min_loss = val_loss
                checkpoint = {
                'epoch': epoch,
        ...
        f=open("Results_linear(pro_chemberta)/chem_aff/{0}.txt".format(_target),'a')
        for i in range(args.num):
            ybar = greedy_decode(transformer, max_len=100, start_symbol=BOS_IDX, EOS_IDX=EOS_IDX, target=_target, device=DEVICE,EMB_SIZE).flatten()
            #print(ybar)

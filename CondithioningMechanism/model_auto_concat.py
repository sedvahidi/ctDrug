...
class ConcatConditioning(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        x = torch.cat([x, cond], dim=-1)  # (S, B, 2E)

        return self.proj(x)  # (S, B, E)
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, nhead,
        ...
        # ✅ Concat conditioning
        self.concat_cond = ConcatConditioning(d_model, d_model)
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Tensor=None , tgt_padding_mask: Tensor=None):
        ...
        # 2. ✅ Concat conditioning
        cond = memory[0]  # (B, E)

        tgt2 = self.concat_cond(tgt, cond)

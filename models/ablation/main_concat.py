...
import numpy as np
import sys
from model_auto_concat import Seq2SeqTransformer, PositionalEncoding, generate_square_subsequent_mask, create_mask
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--num', default=2000, type=int)
    ...
    if args.mode == 'train':
        ...
        transformer = transformer.to(DEVICE)
        transformer, optimizer, scheduler, start_epoch, best_val_loss= load_checkpoint(
                "base_FiLM_best.pth", model=transformer,
                optimizer=optimizer,
                scheduler=scheduler,
                map_location=DEVICE
            )

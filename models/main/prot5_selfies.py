...
def evaluate(model, valid_iter):
    ...
    for idx, _tgt in (enumerate(valid_iter)):
        ...
        if _target is None:
            ...
            targetemb=[]
            for t in _target:
              targetemb.append(pro_smi.loc[pro_smi['seq_id'] == t]['embedding'].values[0])
            target = torch.FloatTensor(targetemb).to(device) 
...
def train_epoch(model, train_iter, optimizer):
    ...
    for idx, _tgt in enumerate(train_iter):
        ...
        if _target is None:
            ...
            targetemb=[]
            for t in _target:
              targetemb.append(pro_emb.loc[pro_emb['seq_id'] == t]['embedding'].values[0])
...
def greedy_decode(model, max_len, start_symbol, target):
    ...
    for i in range(max_len-1):
        ...
        if target == 0:
            ...
            _target = (torch.FloatTensor(pro_smi.loc[pro_smi['seq_id'] == target]['embedding'].values[0])).to(device) ##sed##
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        transformer = transformer.to(DEVICE)
        transformer.load_state_dict(torch.load("model_base_dgsel.h5"))
        ...
        for epoch in range(1, NUM_EPOCHS+1):
            ...
            if (epoch+1)%1==0:
                ...
                if val_loss < min_loss:
                    min_loss = val_loss
                    torch.save(transformer.state_dict(), "model_finetune_pro_seq_dgsel.h5")
                    print('Model saved!')
                ...
                torch.save({
                    ...
                    "scheduler_state_dict": scheduler.state_dict(),
                    "min_loss": min_loss,
                    }, "model_finetune_pro_seq_dgsel_checkpoint.pth" )
        ...
        for i in range(10000):
            ybar = greedy_decode(transformer, max_len=100, start_symbol=s_vocab.bos, target=_target).flatten()
